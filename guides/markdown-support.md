...
2. 第二步：配置 API Key
   - 从 0.1.0 起可直接使用内置免费模型开始对话；如需自配模型，输入 `/model add` 跟随向导
   - 将密钥写入环境变量后在向导中引用
3. 第三步：开始使用
```
...
当你开启流式输出（默认开启）时，Blade 会对“不断增长的 Markdown 文本”进行增量解析与缓存：

- 按行分段，把内容逐步解析成 block（文本/代码块/表格/diff 等）
- 让渲染成本随流式输出平摊，减少大段回复带来的卡顿与闪烁
- 对代码块、表格等结构做状态机处理，避免半成品反复整体重算

**注意**：流式过程中，未闭合的代码块/表格可能会延迟到结构完整后再以最终样式呈现，这是为了保证解析正确性。
